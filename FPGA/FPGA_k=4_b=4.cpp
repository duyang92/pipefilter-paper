...
const uint32_t table_size = (1 << 18);

const uint8_t fp_size = 16;
const uint8_t bytes_per_bucket= (fp_size * 4 + 7) >> 3;
...
void get_a_b_c_FPGA(int capacity) {
	int flag = 0;
...
void generateNextIndex(uint32_t index, uint32_t& next_index,int stage) {
	switch (stage) {
	case(0):
...
bool insertImpl(uint32_t index, uint32_t fingerprint, int stage) {
...
bool insertItem(uint64_t item) {
	...
	uint32_t fingerprint, fingerprint_old;//ָ
