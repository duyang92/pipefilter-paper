#pragma once
#include<string.h>
#include "hashfunction.h"
#include<iostream>
#include<random>
#define MaxNumKicks_V2 500

using namespace std;


typedef struct {
	char* bit_array;
} Bucket_V2;
class VCuckooFilter_V2 {
private:
	size_t capacity;
	size_t fingerprint_size;
	size_t bucket_bits;
	size_t bucket_bytes;
	Bucket_V2* buckets;
	uint32_t mask;//
	uint32_t bm1;
	uint32_t bm2;
	uint32_t random_seed;
public:
	bool is_full;
	bool is_empty;
	size_t size;
	size_t kick_out;
	size_t hash_counter;
	VCuckooFilter_V2(size_t para_capacity, size_t para_fpsize, uint32_t para_bm1, uint32_t para_bm2);

	int insertItem(uint64_t item);

	bool insertItem_cf(uint64_t item);

	bool queryItem(uint64_t item);

	bool queryImpl(const uint32_t index, const uint32_t fingerprint);

	bool insertImpl(uint32_t fp, uint32_t index);

	bool deleteItem(uint64_t item);

	bool deleteImpl(const uint32_t index, const uint32_t fingerprint);

	void get_IF(uint64_t item, uint32_t& fp, uint32_t& index);

	uint32_t get_altindex1(uint32_t index, uint32_t fp);

	uint32_t get_altindex2(uint32_t index, uint32_t fp);

	uint32_t get_altindex3(uint32_t index, uint32_t fp);

	uint32_t swap_fp(uint32_t index, uint32_t fp);

	uint32_t read(uint32_t index, size_t pos);
	void write(uint32_t index, size_t pos, uint32_t fingerprint);
};